...
class Database:
    ...
    def register_database(
        ...
        *,
        ...
        with self._registration_lock:
            ...
            if replaced:
                self._purge_database_state(name)
            ...
            self._evict_lru_databases_locked()
            if connect is None:
    ...
    def ensure_database(
        ...
        *,
        ...
        if self._config.databases.get(name) == db:
            self._touch_dynamic(name)
            return False
        self.register_database(name, db, connect=connect)
        return True
    ...
    def database_scope(
        ...
        **Scope this to a unit of work, never to a request.** Engines/pools are
        ...
        keep it registered.
        """
        self.register_database(name, config, connect=connect)
        try:
    ...
    def _evict_lru_databases_locked(self) -> None:
        ...
        while len(self._dynamic_lru) > limit:
            ...
            self._purge_database_state(victim)
            databases = dict(self._config.databases)
    ...
    def _purge_database_state(self, name: str) -> None:
        """Dispose engines + reset resilience state for one database."""
        self._registry.dispose_database(name)
...
class _BulkPlan:
    ...
    column) so :meth:`Database._bulk_write` / `_bulk_best_effort` stay strategy-agnostic.
    """
