...
async def _apply_local_settings(
    conn: AsyncConnection, settings: Mapping[str, str] | None, *, is_postgres: bool
) -> None:
...
class AsyncConnectionScope:
    ...
    def pipeline(self) -> contextlib.AbstractAsyncContextManager[None]:
        ...
        Usage::
            ...
            async with db.transaction(target=t) as tx, tx.pipeline():
                await tx.execute(INSERT_ORDER, params)
                await tx.execute(INSERT_INBOX, params)
